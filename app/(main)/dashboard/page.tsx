...
const Dashboard = () => {
  ...
  if (!isCoupled) {
    return (
      <div className="min-h-screen flex items-center justify-center md:p-8 p-4">
        <CodeGenerator />
      </div>
    );
  }
    ...
    <div className="flex flex-col p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-2">
          <div>
            <strong>Partner:</strong> {coupleData?.partner?.firstName}{" "}
            {coupleData?.partner?.lastName}
          </div>
          <div>
            <strong>Email:</strong> {coupleData?.partner?.email}
          </div>
          <div>
            <strong>Coupled Since:</strong>{" "}
            {new Date(coupleData?.createdAt || "").toLocaleDateString()}
          </div>
        </div>
      </div>
    </div>
